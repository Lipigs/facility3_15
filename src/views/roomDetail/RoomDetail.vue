<template>
  <div class="roomDetail">
    ...
    <div
      class="reservation"
      v-if="buildingName != null && facilityType != null"
    >
      <van-row class="row">
        <van-col
          span="17"
    ...
    if (timeString.value != "还未选择,请点击预约时段") {
      ...
      addUserReservation(userReservation)
        .then((res) => {
          ...
          if (res.data.status === "OtherExistence") {
            showToast("已被别人预约，请预约其他时段");
            return;
          } else if (res.data.status === "SelfExistence") {
            showToast("你存在预约冲突，请选择其他时段");
            return;
          }
