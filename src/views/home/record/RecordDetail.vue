<template>
  <div class="recordDetail">
    ...
    <div class="top_status" v-if="recordDetail != null">
      ...
      <div style="color: red">
        {{ reservationStatus(recordDetail.status) }}
      </div>
    </div>
    <div>
      <div class="card_center" v-if="recordDetail != null">
        ...
        <van-row class="row" align="center">
          ...
          <van-col span="13" offset="1">{{ recordDetail.roomId }}</van-col>
        </van-row>
        <van-row class="row" align="center">
    ...
    <!-- 底部 -->
    <div style="width: 96vw; margin: 20px auto; right: 0; left: 0">
      <van-button
        round
        type="success"
        color="#fedd02"
...
getUserReservationById(reservationId).then((res) => {
  ...
  getReservationRoomName(res.data.roomId).then((res) => {
    recordDetail.value.roomId = res.data;
  });
...
function reservationStatus(status) {
  ...
  if (status == "AdminAgree") {
    return "待签到使用";
  }
  if (status == "AdminCanceled") {
    return "管理员取消";
  }
