...
function getCanneledList1() {
  List1.value = [];
  List1.value = allList.value.filter((item) => {
    return item.status == "UnConfirmed" ||item.status == "Confirmed";
  });
}
function getCanneledList2() {
  List2.value = [];
  List2.value = allList.value.filter((item) => {
    return item.status == "AdminAgree";
  });
}
