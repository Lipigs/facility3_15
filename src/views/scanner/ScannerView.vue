...
import { SignIn, In } from "../../service/index.js";
...
function signIn(data) {
  ...
  OK_Scanner = 1;
  videoOpacity2.value = 0;
  SignIn(scannerRes.value.roomId)
    .then((res) => {
      if (res.data.code === 200 && res.data.status === true) {
        tipMsg.value = "正在签到！";
        showDialog({
          message: `是否签到场地：${res.data.item}`,
          theme: "round-button",
          showCancelButton: true,
        })
          .then(() => {
            In(res.data.totalCount) 
              .then((res) => {
                tipMsg.value = res.data.message;
                showDialog({
                  message: tipMsg.value,
                  theme: "round-button",
                }).then(() => {
                  clickIndexLeft();
                });
